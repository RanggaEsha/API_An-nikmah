...
def add_transaction(user_id,address,fullname,phone_number):
    cur = conn.cursor()
    try:
        cur.execute("INSERT INTO transactions (user_id,address,fullname,phone_number,created_at) VALUES (%s,%s,%s,%s,%s) RETURNING id",(user_id,address,fullname,phone_number,datetime.now()))
        conn.commit()
...
def get_transaction_details_by_transaction_id(transaction_id):
    cur = conn.cursor()
    try:
        cur.execute("SELECT * FROM transaction_details WHERE transaction_id = %s",(transaction_id,))
        data = cur.fetchall()
        list_data = []
        if data:
            for item in data:
...
def get_transaction_details_by_user_id_and_product_id(transaction_id,product_id):
    cur = conn.cursor()
    try:
        cur.execute('SELECT id,user_id,product_id,quantity FROM treansaction_details WHERE user_id = %s AND product_id=%s', (transaction_id,product_id))
        conn.commit()
        data = cur.fetchone()
        if data is not None:
            new_data = {
...
def add_transaction_details(transaction_id,product_id,quantity,price):
    cur = conn.cursor()
    try:
        cur.execute("INSERT INTO transaction_details (transaction_id,product_id,quantity,price,created_at) VALUES (%s,%s,%s,%s,%s)",(transaction_id,product_id,quantity,price,datetime.now()))
        conn.commit()
    except Exception as e:
        conn.rollback()
        raise e
